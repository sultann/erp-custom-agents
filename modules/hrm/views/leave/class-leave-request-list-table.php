<?php
namespace Pluginever\ERPCA\HRM\Views\Leave;
...
class Leave_Requests_List_Table extends \WP_List_Table {
	...
	function column_name( $item ) {
		...
		if($this->is_user_leave_agent()){
			if(in_array($item->status, [11,2])) {
				$actions['reject']   = sprintf( '<a class="erp-hr-leave-reject-btn1" data-id="%s" href="%s">%s</a>', $item->id, $reject_url, __( 'Reject', 'erp' ) );
